...
- `references/semgrep-rules.md` - Pattern-based security rule development
- `references/sonarqube-config.md` - Quality gates and profiles
- `references/codeql-setup.md` - Query development and workflows

## Templates & Assets

- `assets/semgrep-config.yml` - Production-ready Semgrep configuration
- `assets/sonarqube-settings.xml` - SonarQube quality profile template
- `scripts/run-sast.sh` - Automated SAST execution script
...
```bash
# Automated SAST setup script (if available in your CI/CD)
...
```yaml
# See references/semgrep-rules.md for detailed examples (if configured)
rules:
